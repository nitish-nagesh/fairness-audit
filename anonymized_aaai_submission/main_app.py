...
if st.button("Run Prediction and Show Fairness Plot"):
    ...
    st.image("fig_criminal_justice_yhat_rf.png", use_container_width=True, caption="Criminal Justice Fairness Decomposition after Random Forest prediction")
    
import base64

random_forest_prompt = """
You are analyzing a fairness decomposition plot produced after applying a Random Forest classifier on the Criminal Justice dataset.
...
if st.button("Ask GPT-4o to Explain Prediction Plot"):
    with st.spinner("Calling GPT-4o Vision..."):
        ...
        with open("fig_criminal_justice_yhat_rf.png", "rb") as image_file:
...
# --- Criminal Justice Outcome Control Results Section ---
st.markdown("## Fairness Outcome Control")

# Persistent button using session_state
if "show_criminal_justice" not in st.session_state:
    st.session_state["show_criminal_justice"] = False

if st.button("Show Criminal Justice Outcome Control Results"):
    st.session_state["show_criminal_justice"] = True

if st.session_state["show_criminal_justice"]:
    criminal_justice_data = [
    ...
    criminal_justice_df = pd.DataFrame(criminal_justice_data, columns=["measure", "value", "sd", "outcome"])
    st.dataframe(criminal_justice_df)
    ...
    for i, policy in enumerate(criminal_justice_df["outcome"].unique()):
        sub_df = criminal_justice_df[criminal_justice_df["outcome"] == policy].set_index("measure").loc[measures]
    ...
    ax.set_title("Criminal Justice Decomposition by Policy")
    ax.legend()
    st.pyplot(fig)

    # --- GPT-4o Explanation ---
    st.markdown("### GPT-4o Explanation")
    if st.button("Explain Criminal Justice Outcome Control", key="explain_criminal_justice_oc"):
        from openai import OpenAI
        client = OpenAI(api_key=openai.api_key)
        summary = criminal_justice_df.groupby("outcome").apply(lambda g: "\n".join(
