...
# Read CSV
df_raw <- read.csv("mimic_synthetic_data_1000_samples_DECAF.csv")
...
if ("los_seconds" %in% names(df)) {
  df$los_seconds <- as.numeric(df$los_seconds)
  df$los_seconds <- as.integer(df$los_seconds >= 345600)
}
...
# Save and print
write.csv(result_df, "fairness_Output.csv", row.names = FALSE)
